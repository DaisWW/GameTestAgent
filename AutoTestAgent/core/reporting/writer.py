...
def write_report(
    ...
    for s in history:
        action    = s.get("action", "")
        params    = s.get("params", {})
        reasoning = s.get("reasoning", "")[:50]
        label     = s.get("element_label", "") or str(params)
        changed   = "✓" if s.get("success") else "—"
        elem_id   = params.get("id", "")
        lines.append(
            f"| {s.get('step', '')} | `{action}` | [{elem_id}] {label} | {reasoning} | {changed} |"
        )
    ...
    if new_bugs:
        ...
        lines.append("|--------|------|------|------|")
        for b in new_bugs:
            sev  = sev_map.get(b.get("severity", ""), "⚪")
            cat  = b.get("category", "")
            desc = (b.get("description") or "")[:60]
            ph   = (b.get("page_hash") or "")[:8]
    ...
    lines += [
        ...
        "## 导航图统计",
        "",
        f"- 已知页面: **{nav_stats.get('pages', 0)}**",
        f"- 页面转移: **{nav_stats.get('transitions', 0)}**",
        f"- 累计探索元素: **{nav_stats.get('visited_elements', 0)}**",
        "",
    ]
